...
impl Config {
    ...
    pub fn from_env() -> Self {
        ...
        if let Ok(enable_hold) = env::var("MARKET_ENABLE_HOLD_PERIOD") {
            config.market.enable_hold_period = enable_hold
                .parse()
                .unwrap_or(config.market.enable_hold_period);
        }

        if let Ok(hold_hours) = env::var("MARKET_HOLD_PERIOD_HOURS") {
            config.market.hold_period_hours = hold_hours
                .parse()
                .unwrap_or(config.market.hold_period_hours);
        }

        if let Ok(kelly_frac) = env::var("MARKET_KELLY_FRACTION") {
            let parsed_kelly = kelly_frac.parse().unwrap_or(config.market.kelly_fraction);
            // Clamp to reasonable bounds
            config.market.kelly_fraction =
                parsed_kelly.max(0.0).min(config.market.max_kelly_fraction);
        }

        if let Ok(max_kelly) = env::var("MARKET_MAX_KELLY_FRACTION") {
            config.market.max_kelly_fraction = max_kelly
                .parse()
    ...
    fn validate(&mut self) {
        ...
        if self.market.hold_period_hours < 0.0 {
            eprintln!(
                "⚠️  Invalid hold_period_hours: {}, using default",
                self.market.hold_period_hours
            );
            self.market.hold_period_hours = 1.0;
        }

        // Ensure Kelly fraction is within bounds
        if self.market.kelly_fraction < 0.0
            || self.market.kelly_fraction > self.market.max_kelly_fraction
        {
            eprintln!(
                "⚠️  Invalid kelly_fraction: {}, using default",
                self.market.kelly_fraction
            );
        ...
        if self.market.max_kelly_fraction < 0.0 || self.market.max_kelly_fraction > 2.0 {
            eprintln!(
                "⚠️  Invalid max_kelly_fraction: {}, using default",
                self.market.max_kelly_fraction
            );
