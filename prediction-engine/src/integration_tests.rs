...
mod tests {
    ...
    async fn test_numerical_stability_under_stress() -> Result<()> {
        ...
        for i in 0..event_count {
            let event_id = create_test_event(&pool, &format!("Stability Stress Event {}", i)).await?;
        ...
        for _ in 0..total_ops {
            ...
            let current_prob: f64 = sqlx::query_scalar("SELECT market_prob FROM events WHERE id = $1")
                .bind(event_id)
                .fetch_one(&pool)
                .await?;
        ...
        assert!(executed_cycles > 0, "No trades executed during stability test");
        println!(
