    ...
    return (
        <div class="flex flex-col h-full bg-bb-bg text-xs font-mono">
            <div class="grid grid-cols-12 gap-2 p-2 border-b border-bb-border text-bb-muted bg-bb-panel">
                <div class="hidden sm:block col-span-1">ID</div>
                <div class="col-span-8 sm:col-span-6">EVENT</div>
                <div class="col-span-4 sm:col-span-2 text-right">PROB</div>
                <div class="hidden sm:block col-span-3 text-right">CLOSE / OUT</div>
            </div>
            ...
            <div class="flex-1 overflow-auto custom-scrollbar">
                <For each={marketStore.state.markets}>
                    {(market) => (
                        ...
                        >
                            <div class="hidden sm:block col-span-1 text-bb-muted">{market.id.toString().substring(0, 4)}</div>
                            <div class="col-span-8 sm:col-span-6 truncate text-bb-text">{market.title}</div>
                            <div class="col-span-4 sm:col-span-2 text-right text-market-up font-bold">
                                {market.market_prob != null ? `${(Number(market.market_prob) * 100).toFixed(1)}%` : "--"}
                            </div>
                            <div class="hidden sm:block col-span-3 text-right text-bb-muted uppercase truncate">
