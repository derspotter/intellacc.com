...
export const getLeaderboardNetwork = (limit = 10) => api.leaderboard.getNetwork(limit);
...
export const createMarketQuestion = (payload) => api.marketQuestions.create(payload);

export const listMarketQuestions = ({ status = null, mine = false, limit = 20, offset = 0 } = {}) =>
  api.marketQuestions.list({ status, mine, limit, offset });

export const getMarketQuestionReviewQueue = ({ limit = 20 } = {}) =>
  api.marketQuestions.getReviewQueue({ limit });

export const submitMarketQuestionReview = (submissionId, vote, note = null) =>
  api.marketQuestions.submitReview(submissionId, { vote, note });
...
export const updateProfile = ({ username, display_name, bio, profile_visibility }) =>
  api.users.updateProfile({ username, display_name, bio, profile_visibility });
