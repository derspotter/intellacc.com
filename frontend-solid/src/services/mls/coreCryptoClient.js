import init, { MlsClient, init_logging } from '../../pkg/openmls-wasm/openmls_wasm.js';
import { onMlsMessage, onMlsWelcome } from '../socket.js';

// Solid's socket service exposes onMlsMessage/onMlsWelcome instead of registerSocketEventHandler.
const registerSocketEventHandler = (event, cb) => {
  if (event === 'mls-message') return onMlsMessage(cb);
  if (event === 'mls-welcome') return onMlsWelcome(cb);
  return () => {};
};
...
class CoreCryptoClient {
    ...
    async acceptStagedWelcome(stagingId) {
        ...
        if (fingerprintWarnings.length > 0) {
            ...
            // TODO: wire up messagingStore for fingerprint warning UI
            ...
            for (const msg of pending) {
                ...
                try {
                    if (msg.message_type === 'welcome') {
                        ...
                        // Auto-accept DM invitations and invitations from followed users
                        const isDM = staged.groupId && staged.groupId.startsWith('dm_');
                        let welcomeOutcome = (followsSender || isDM) ? 'accepted' : 'pending';
                        if (messageId) {
