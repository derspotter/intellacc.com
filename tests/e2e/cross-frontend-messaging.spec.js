...
test.describe('Cross-Frontend Messaging Interop', () => {
  ...
  test('Solid user and Van user can exchange messages', async ({ browser }) => {
    ...
    await openDmOnSolid(solidPage, vanUser.username, 90000, solidUser);
    ...
    await sendFromSolid(solidPage, msgSolidToVan, {
      ...
      deviceId: solidUser.device_public_id || solidUser.deviceId || solidUser.device_id || null,
    ...
    await openVanMessagesUnlocked(vanPage, vanUser.password, vanUser);
    await acceptWelcomeIfPresent(vanPage);
    await startDmFromVan(vanPage, solidUser.username, vanUser, solidUser.id);
    ...
    await sendFromVan(vanPage, msgVanToSolid, vanUser, solidUser.id);

    // Solid waits for Van reply.
    await openDmOnSolid(solidPage, vanUser.username, 90000, solidUser);
    await waitForSolidIncomingText(solidPage, msgVanToSolid, 90000, {
      conversationId: vanConversationId || solidConversationId,
      authToken: solidUser.token
    });
